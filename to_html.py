...
try:
    ...
    cur.execute ("SELECT dvd.dvd_id, dvd.title FROM dvd ORDER by dvd.title;")
...
# DVDs by tag
    ...
    for tag in tag_list:
        cur.execute ("SELECT dvd.dvd_id, dvd.title FROM dvd_tags INNER JOIN dvd "
                     "ON dvd.dvd_id = dvd_tags.dvd_id "
                     "WHERE dvd_tags.tag = \"{}\" ORDER BY dvd.title;".format (tag[0]))
        dvd_list = cur.fetchall ()
...
#        print ("<h3 id=\"movie{}\">{} ({}){}</h3>".format (movie[0], process_name (movie[1]), movie[2], is_full_length (movie[3])));
        ...
        print ("<ul>")
        cur.execute ("SELECT dvd.title, dvd.dvd_id FROM dvd INNER JOIN dvd_contents "
                     "ON dvd.dvd_id = dvd_contents.dvd_id "
                     "WHERE dvd_contents.movie_id = {} "
                     "ORDER BY dvd.title;".format (movie[0]));
        dvd_list = cur.fetchall ()
        if len (dvd_list) == 0:
            print ("<li>ERROR: not found on any DVD!</li>")
        for dvd in dvd_list:
            print ("<li>on <i><a href=\"#dvd{}\">{}</a></i></li>".format (dvd [1], process_name (dvd[0])))
        cur.execute ("SELECT person.person FROM person WHERE person.movie_id = {} ORDER BY person.person;".format (movie [0]))
...
# People
    ...
    cur.execute ("select COUNT(*), person from person GROUP BY person HAVING count(*) > 1 ORDER BY count(*) DESC, person;")
    rows = cur.fetchall ()
    for row in rows:
        ...
        cur.execute ("select m.movie_id, m.name, m.year FROM movie m INNER JOIN person p ON p.movie_id = m.movie_id "
                     "WHERE p.person = {} ORDER BY m.year, m.sort_name;".format(con.escape (row [1])))
        movies = cur.fetchall ()
        print ("<ul>")
        for movie in movies:
