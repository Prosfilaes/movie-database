...
try:
    ...
    cur.execute ("SELECT person, bacon_num FROM bacon where table_num = 1;")
    prior_bacon_nums = dict (cur.fetchall ())
    if insert:
        cur.execute ("DELETE FROM bacon where table_num = 1;")
...
#    total_person_set = set ([a[0] for a in cur.fetchall()])
    ...
    for x in bacon_list:
        if insert:
            cur.execute ("INSERT into bacon VALUES (1, {}, {});".format (
                    con.escape (x[0]), x[1]));
