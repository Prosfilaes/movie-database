...
try:
    ...
    cur.execute ("SELECT DISTINCT dvd.dvd_id, dvd.title FROM dvd "
                 "INNER JOIN dvd_contents dc ON dc.dvd_id = dvd.dvd_id "
                 "INNER JOIN movie m ON m.movie_id = dc.movie_id "
                 "WHERE m.is_full_length = TRUE ORDER BY dvd.title;")
...
#        print ("<h3 id=\"movie{}\">{} ({}){}</h3>".format (movie[0], process_name (movie[1]), movie[2], is_full_length (movie[3])));
        ...
        print ("<ul>")
        cur.execute ("SELECT dvd.title, dvd.dvd_id FROM dvd "
...
# People
    ...
    cur.execute ("select COUNT(*), person FROM person "
    ...
    for row in rows:
        ...
        cur.execute ("select m.movie_id, m.name, m.year FROM movie m INNER JOIN person p ON p.movie_id = m.movie_id "
                     "WHERE p.person = {} AND m.is_full_length ORDER BY m.year, m.sort_name;".format(con.escape (row [1])))
        movies = cur.fetchall ()
        print ("<ul>")
        for movie in movies:
