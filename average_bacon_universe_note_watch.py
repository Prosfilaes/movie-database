...
try:
    ...
    cur.execute ("SELECT person, bacon_num FROM actorbacon WHERE table_num = 4;")
    ...
    if True:
        cur.execute ("INSERT INTO tmp_p2p "
                     ...
                     "INNER JOIN movie m on a2.movie_id = m.movie_id AND m.have_watched "
...
#    exit ()
    ...
    root = "Dana Hill (I)"
    cur.execute ('SELECT person_id FROM tmp_pid WHERE person = {};'.format (con.escape(root)))
    root_id = cur.fetchone()[0]
    (dana_person_set, people_tree) = calculate_person_data (root_id)
    num_people = len (dana_person_set)
    total_person_set = set ()
    bacon_list = [(root, average_bacon_num (people_tree, num_people))]
    
    cur.execute ("SELECT person FROM notable_actor;");
    notable_people = cur.fetchall()
    printed_num_people = len (notable_people) + 1
    print ("Total number of people: (at most) ", printed_num_people, " out of ", num_people)
    sys.stdout.flush()

    for person in notable_people:
        cur.execute ('SELECT person_id FROM tmp_pid WHERE person = {};'.format (con.escape(person)))
        person_id = cur.fetchone()[0]
        if person_id != root_id and person_id in dana_person_set:
            bacon_list.append ((person[0], average_bacon_num (calculate_person_data (person_id)[1], num_people)))

    data_collection_time = time.time ()
    bacon_list.sort (key=lambda x: x[1])
    if insert:
        cur.execute ("DELETE FROM actorbacon WHERE table_num = 4;")
    for x in bacon_list:
        if insert:
            cur.execute ("INSERT into actorbacon VALUES (4, {}, {});".format (
                    con.escape(x[0]), x[1]));
